...
const MyEventsCard = ({MyEvent}) => {
    const {eventTitle,name,email,eventDate,eventTime,location,description,attendeeCount} = MyEvent;
    return (
        <div className='Event_card bg-white  text-black shadow-lg rounded-md px-4 py-8 border-1 border-sky-500 hover:bg-[#00A4EF] hover:text-white transition-all'>
                   ...
                   <div className="card_button flex  gap-x-6 justify-end ">
                                  <Link className='primary_btn flex justify-center items-center text-xl'>  <FaEdit></FaEdit> </Link>
                                  <Link className='btn flex justify-center items-center text-xl bg-red-400 text-black'> <MdDelete></MdDelete>  </Link>
                   </div> 
